...
const InfoWidget: React.FC = () => {
  ...
  useEffect(() => {
    const updateDateTime = () => {
      const now = new Date();
      const optionsDateDetailed: Intl.DateTimeFormatOptions = { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' };
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            ...
            fetchWeatherData(-6.9740, 107.6322); // Koordinat Bojongsoang
          },
  ...
  const getWeatherIcon = (iconName: string) => {
    switch (iconName) {
      case 'sunny': return 'https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/60001859600/original/sunny.gif?1682704381'; // Contoh GIF
      case 'rainy': return 'https://i.gifer.com/VgC.gif'; // Contoh GIF
      case 'cloudy': return 'https://i.gifer.com/origin/79/79b47e2524d775a6c3f3f009f4dd71a3_w200.gif'; // Contoh GIF
      case 'windy': return 'https://i.gifer.com/origin/ff/ff37986064f26197b0a887019f6a5d42_w200.gif'; // Contoh GIF
      default: return 'https://via.placeholder.com/80/A0AEC0/FFFFFF?text=Weather'; // Placeholder
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row bg-gray-50 bg-opacity-70 backdrop-blur-md rounded-2xl shadow-xl p-6 gap-6 w-full max-w-lg md:max-w-3xl mx-auto border border-gray-200">
      ...
      <div className="flex-1 flex flex-col items-start justify-center text-gray-800 border-b md:border-b-0 md:border-r border-gray-300 pb-4 md:pr-6 md:pb-0">
        ...
        <p className="text-xl md:text-2xl font-semibold mt-1 leading-none">{currentDay}</p> {/* WEDNESDAY */}
        <p className="text-4xl md:text-5xl font-bold mt-1 leading-none text-gray-900">{currentDateNum}</p> {/* 15 */}
        <p className="text-xl font-medium mt-1 uppercase text-gray-700">{currentMonthYear}</p> {/* NOVEMBER 2025 */}
        ...
        {isLoadingWeather ? (
          ...
          <>
            <div className="w-20 h-20 flex items-center justify-center mb-2">
              <img src={getWeatherIcon(weather.icon)} alt={weather.condition} className="w-full h-full object-contain" />
